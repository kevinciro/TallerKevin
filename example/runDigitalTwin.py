...
with torch.no_grad():
    x_sim_pop = myDigitalTwin.nn_solution(
        getInitSSFromFile(initCGM, fileInitStates).reshape(1, -1),
        myDigitalTwin.u_pop[:, [0], :],
        None,
        is_pers=False,
    )
    x_sim_ind = myDigitalTwin.nn_solution(
        getInitSSFromFile(initCGM, fileInitStates).reshape(1, -1),
        myDigitalTwin.u_pop[:, [0], :],
        myDigitalTwin.u_ind[:, [0], :],
        is_pers=True,
    )

df_scenario_subj["cgm_AIPop"] = scale_inverse_Q1(
    x_sim_pop[:, 0, [0]], population_model_path
)
df_scenario_subj["cgm_AIDT"] = scale_inverse_Q1(
    x_sim_ind[:, 0, [0]], population_model_path
)
...
color = "tab:red"
ax2_carbs.plot(
    df_scenario_subj.loc[df_scenario_subj.input_meal_carbs_ODEPop != 0].index,
    df_scenario_subj.loc[
        df_scenario_subj.input_meal_carbs_ODEPop != 0, "input_meal_carbs_ODEPop"
    ],
    "o",
    color=color,
)
