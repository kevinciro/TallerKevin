...
class ForwardEulerSimulator(nn.Module):
    ...
    def forward(self, x0_batch: torch.Tensor, u_batch: torch.Tensor,u_batch_ind, is_pers = True) -> torch.Tensor:
...
class CGMIndividual(nn.Module):
    def __init__(self,hidden_compartments,init_small=True):
        ...
        for i in range(len(hidden_compartments['model'])-2):
            layers_model.append(nn.Linear(hidden_compartments['model'][i], hidden_compartments['model'][i+1]))
    ...
    def forward(self,in_x, u_pop,u_ind):
        q1, q2, s1,s2, I, x1, x2, x3, c2, c1 = in_x[..., [0]],in_x[..., [1]],in_x[..., [2]],\
                                             in_x[..., [3]],in_x[..., [4]],in_x[..., [5]],\
                                             in_x[..., [6]],in_x[..., [7]],in_x[..., [8]],in_x[..., [9]]
...
class DigitalTwin:
    def __init__(self,subjectID,df_subj, states,inputs_pop,input_ind,popModel,pathModel,device='cpu'):
    ...
    def preprocess(self,states,inputs_pop,input_ind):
        ...
        self.scaler_featsRobust = load(open(self.pathModel+'/scaler_robust.pkl', 'rb'))
        ...
        x_est_test, u_pop_test = scaler_pop(x_est_test,u_pop_test,self.popModel,False)

        # Scale new inputs
        u_ind_test[:,idx_robust] = self.scaler_featsRobust.transform(u_ind_test[:,idx_robust])

        self.u_pop_test = u_pop_test.reshape(-1,sim_time_test,len(inputs_pop))[0,batch_idx,:]
        self.u_ind_test = u_ind_test.reshape(-1,sim_time_test,len(input_ind))[0,batch_idx,:]

        self.u_pop = torch.tensor(self.u_pop_test[[0],batch_idx.T],dtype=torch.float32).to(self.device)
        self.u_ind = torch.tensor(self.u_ind_test[[0],batch_idx.T],dtype=torch.float32).to(self.device)
    ...
    def setup_simulator(self,n_neurons_pop,hidden_compartments,ts=5):
        ...
        self.individual_model.load_state_dict(torch.load(self.pathModel+'individual_model.pt'))

        # Load population model
        self.ss_pop_model = CGMOHSUSimStateSpaceModel_V2(n_feat=n_neurons_pop)
        self.ss_pop_model.to(self.device)
        self.ss_pop_model.load_state_dict(torch.load(glob.glob(self.popModel+'*pt')[0]))
        ...
        # Simulator
        self.nn_solution = ForwardEulerSimulator(self.ss_pop_model,self.individual_model, self.popModel,ts=ts)
