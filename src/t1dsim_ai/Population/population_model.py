...
class CGMOHSUSimStateSpaceModel_V2(nn.Module):
    ...
    def __init__(self, n_feat=None, lookback_inputs=[None,None], scale_dx=1.0, init_small=True):
        super(CGMOHSUSimStateSpaceModel_V2, self).__init__()
        self.n_feat = n_feat
        self.scale_dx = scale_dx

        self.input_insulin = 1 if lookback_inputs[0] == None else int(lookback_inputs[0]*12)
        self.input_carbs = 1 if lookback_inputs[1] == None else int(lookback_inputs[1]*12)
        ...
        if init_small:
            ...
            networks = {'S1':self.net_dS1,'S2':self.net_dS2,'I':self.net_dI,
                        'X1':self.net_dX1,'X2':self.net_dX2,'X3':self.net_dX3,
                        'Q1':self.net_dQ1,'Q2':self.net_dQ2,'C1':self.net_dC1,
                        'C2':self.net_dC2}
        ...
        #print(npa)
        #exit()
        #self.net_dU_G2[0].bias = None
        #self.net_dU_G2[1].bias = None
    ...
    def forward(self, in_x, in_u):
        ...
        #u_I, u_carbs = in_u[..., 0:self.lookback], in_u[..., self.lookback:]
        ...
        # NN3(s2,I)
        in_3 = torch.cat((s2,I), -1)  # concatenate
        dI = self.net_dI(in_3)

        # NN4(I,x1)
        in_4 = torch.cat((I,x1), -1)  # concatenate
        dX1 = self.net_dX1(in_4)

        # NN5(I,x2)
        in_5 = torch.cat((I,x2), -1)  # concatenate
        dX2 = self.net_dX2(in_5)

        # NN6(I,x3)
        in_6 = torch.cat((I,x3), -1)  # concatenate
